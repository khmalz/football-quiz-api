...
import { addDocument, retrieveDataSubByDocId } from "../lib/firestore/service";
...
app.post("/user", async (c, next) => {
   const { name } = await c.req.json();

   await addDocument("user", { name });
   return c.json({ status: true, statusCode: 200, message: "Success to add user" });
});
